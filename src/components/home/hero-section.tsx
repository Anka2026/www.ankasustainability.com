...
import { BarChart3, FileCheck, Laptop2, Leaf, ListChecks } from "lucide-react";
...
const PANEL_ICONS = [Leaf, BarChart3, FileCheck, Laptop2] as const;
...
export function HeroSection({ t }: Props) {
  ...
  return (
    ...
    >
      ...
      <Container className="relative z-[1] max-w-7xl pt-10 pb-9 sm:pt-11 sm:pb-10 md:pt-12 md:pb-10 xl:max-w-[86rem] 2xl:max-w-[92rem]">
        <div className="grid items-stretch gap-8 sm:gap-9 md:gap-10 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1fr)] lg:items-center lg:gap-9 xl:gap-10 2xl:gap-11">
          <div className={cn(HERO_LEFT_RAIL_HOME, "min-w-0 max-w-[min(100%,32rem)] sm:max-w-[36rem] lg:max-w-[min(100%,40rem)]")}>
            ...
            <h1
              ...
              className="mt-3.5 text-balance text-pretty text-[1.68rem] font-semibold leading-[1.1] tracking-[-0.03em] text-slate-900 sm:mt-4 sm:text-[2.1rem] sm:leading-[1.08] md:text-[2.35rem] lg:text-[2.45rem]"
            >
              {t("hero.title")}
            </h1>
            ...
            <div className="mt-5 flex w-full min-w-0 flex-col gap-3 sm:mt-6 sm:max-w-lg sm:flex-row sm:items-stretch sm:gap-3.5 md:max-w-none">
              <AppButton
                asChild
          ...
          <div className="order-2 w-full min-w-0 self-stretch lg:order-none">
            ...
            >
              ...
              <div className="relative flex flex-1 flex-col p-5 sm:p-6 md:p-7">
                ...
                <div className="mt-4 grid min-h-0 flex-1 grid-cols-1 gap-2.5 sm:mt-5 sm:grid-cols-2 sm:gap-3">
                  {panelItems.slice(0, 4).map((label, idx) => {
                    const Icon = PANEL_ICONS[idx] ?? Leaf;
                    return (
                      <div
                        key={label}
                        className="group flex min-h-[3.75rem] rounded-2xl border border-slate-200/90 bg-white/75 p-3.5 shadow-[0_10px_28px_-20px_rgba(15,23,42,0.3)] ring-1 ring-inset ring-slate-900/[0.03] transition-all duration-200 hover:-translate-y-px hover:border-primary/20 hover:bg-white hover:shadow-[0_16px_40px_-28px_rgba(8,145,178,0.28)]"
                      >
